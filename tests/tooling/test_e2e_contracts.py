...
@pytest.mark.parametrize("mutation", ["provider", "model", "business", "ssh", "volume", "port", "gpu", "secret"])
def test_e2e_isolation_fails_closed(tmp_path: Path, mutation: str) -> None:
    ...
    if mutation == "provider":
        ...
        api["environment"]["IP_BUSINESS__PASSWORD"] = "synthetic"
    elif mutation == "ssh":
...
@pytest.mark.parametrize(("markers", "valid"), [
    (["e2e", "integration"], True), (["e2e"], False),
    (["e2e", "integration", "storage"], False),
    (["e2e", "external"], True), (["e2e", "external", "gpu"], True),
])
def test_e2e_partition_contract_before_deselection(pytester: pytest.Pytester, markers: list[str], valid: bool) -> None:
    ...
    assert result.ret == (pytest.ExitCode.NO_TESTS_COLLECTED if valid else pytest.ExitCode.USAGE_ERROR)


def test_test_image_and_evidence_are_not_production_inputs() -> None:
    source = (ROOT / "docker/Dockerfile.api").read_text()
    assert "COPY tests" not in source and "e2e" not in source
