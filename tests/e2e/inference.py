...
async def wait_for_sql(script: Script, request: CompletionRequest, release: asyncio.Event) -> None:
    if (
        script.scenario is Scenario.CHAOS
...
def application() -> FastAPI:
    ...
    async def completion(request: CompletionRequest) -> JSONResponse:
        try:
            await wait_for_sql(script, request, release)
    ...
    async def rerank(request: RerankRequest) -> RerankResult:
        ...
        return RerankResult(
            ...
            inference_ms=0,
            metadata=metadata(),
            scores=[0.9 for _ in request.passages],
        )
