...
@pytest.fixture
def chaos_mcp(
    migration_stack: DatabaseStack, checkpoint_setup: None, milvus_stack: MilvusStack,
    tmp_path: Path,
) -> Iterator[ChaosMcp]:
    settings = migration_stack.settings.model_copy(update={"mcp_auth_token": SecretStr(secrets.token_urlsafe(24))})
    ...
    command = [
        migration_stack.docker, "compose", "-p", settings.compose_project_name,
        "--project-directory", str(ROOT), "--env-file", str(ROOT / ".env.deployment.example"),
        "-f", str(ROOT / "docker-compose.yml"), "-f", str(overlay), "--profile", "core",
    ]
    directory = milvus_stack.directory / "step410-chaos"
    recorder = CommandRecorder(
        directory=directory / "commands", cwd=ROOT, environment=process_environment(settings),
        secrets=[settings.postgres_superuser_password, settings.bootstrap.app_password,
                 settings.bootstrap.mcp_password, settings.bootstrap.etl_password, settings.mcp_auth_token],
    )
    with retain_primary_failure([
        ...
        recorder.run("start-mcp", [*command, "up", "-d", "--no-deps", "--wait", "--wait-timeout", "90", "mcp"], timeout=120)
        yield ChaosMcp(command, recorder, MCPSettings(base_url=f"http://127.0.0.1:{port}/mcp", auth_token=settings.mcp_auth_token), directory)
    assert foreign_snapshot(migration_stack.docker) == before


async def test_mcp_killed_midturn(
    chat: Harness, chaos_mcp: ChaosMcp, harness: RetrievalHarness,
    ...
    try:
        async with asyncio.timeout(60), asyncio.TaskGroup() as tasks:
            request = tasks.create_task(chat.client.post(chat.url, json={"content": "请分析2026年8月经营数据与退款政策"}))
            await data_entered.wait()
            await knowledge_ready.wait()
            await asyncio.to_thread(chaos_mcp.recorder.run, "kill-midturn", [*chaos_mcp.command, "stop", "-t", "1", "mcp"])
            stopped.set()
        response: httpx.Response = request.result()
        ...
        evidence = await chat.client.get(f"/api/v1/conversations/{chat.cid}/turns/{result['id']}/evidence")
        assert evidence.status_code == 200
        assert evidence.json()["knowledge"]["id"] == result["evidence_refs"]["knowledge_snapshot_id"]
        chaos_mcp.directory.joinpath("response.json").write_text(json.dumps({"response": result, "evidence": evidence.json()}, ensure_ascii=False, indent=2))
    finally:
        await mcp.aclose()
        await graph.aclose()
