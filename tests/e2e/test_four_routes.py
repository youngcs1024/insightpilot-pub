...
async def test_data_only(session: Session) -> None:
    ...
    assert turn.answer.sql
    assert turn.answer.assumptions
    ...
    assert "mcp_execute" in calls
    assert "retrieve" not in calls
    assert "retrieval_encode" not in calls
...
async def test_knowledge_only(session: Session) -> None:
    ...
    assert "mcp_execute" not in calls
    assert "mcp_schema" not in calls
    assert "retrieval_encode" in calls
    evidence = await session.evidence(turn)
    assert evidence.data is None
    assert evidence.knowledge.knowledge.chunks
    ...
    status = await session.complete()
    assert status.embed_query > 0
    assert status.rerank > 0
...
async def test_both(session: Session) -> None:
    ...
    assert [Decimal(str(row[1])) for row in evidence.data.data.rows] == [
        Decimal("0.5"),
        Decimal("1"),
    ]
...
async def test_clarify(session: Session) -> None:
    ...
    assert turn.answer.abstained
    assert turn.clarification
    assert turn.content
    assert not turn.answer.claims
...
async def followup(session: Session, *, restart: bool) -> None:
    ...
    if restart:
        ...
        assert [
            item.model_dump(exclude={"request_id"}) for item in (await session.history()).items
        ] == [item.model_dump(exclude={"request_id"}) for item in history.items]
    ...
    assert len((await session.history()).items) == TWO_TURN_MESSAGES
    await session.complete()
...
async def test_evidence_retrievable_later(session: Session) -> None:
    ...
    try:
        ...
        assert restored.data.data.sql
        assert all(chunk.chunk_id for chunk in restored.knowledge.knowledge.chunks)
