...
def parent_context(route: Route, *, data_error: Exception | None = None,
                   knowledge_error: Exception | None = None, empty: bool = False) -> RuntimeContext:
    ...
    decision = RouteDecision(
        ...
        knowledge_intent="核查2026年8月退款政策" if route in {Route.KNOWLEDGE_ONLY, Route.BOTH} else "",
    ...
    if route in {Route.KNOWLEDGE_ONLY, Route.BOTH} and not knowledge_error and not empty:
        responses.append(draft(retrieval.candidates[0].chunk_id))
    ctx = context(responses=responses, mcp_results=[data_error] if data_error else None)
    return replace(
        ctx,
        conversations=AsyncMock(prepare=AsyncMock(return_value=PreparedContext(
