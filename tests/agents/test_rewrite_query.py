...
async def test_standalone_question_does_not_inherit_history_or_call_llm() -> None:
    ...
    output = await invoke(
        ctx,
        inputs(question="促销规则是什么\uff1f", time_scope=None, knowledge_history=[topic(2024)]),
    )
...
async def test_followup_explicit_time_overrides_history() -> None:
    ...
    service = FakeRetrieval(ranked())
    ctx = replace(
        context(responses=[rewrite(prior, "2026年8月退货政策中的运费规则")]), retrieval=service
    )
    output = await invoke(
        ctx,
        inputs(question="那2026年8月的运费呢\uff1f", time_scope=None, knowledge_history=[prior]),
    )
...
async def test_yearless_followup_preserves_history_year() -> None:
    prior = topic(2024, 7)
    ctx = replace(
        context(responses=[rewrite(prior, "2024年8月退货政策")]), retrieval=FakeRetrieval(ranked())
    )
    output = await invoke(
        ctx, inputs(question="那8月呢\uff1f", time_scope=None, knowledge_history=[prior])
    )
...
@pytest.mark.parametrize(
    "question", ["2026年13月政策", "2026年2月30日政策", "2026年7月和13月政策", "去年同期政策"]
)
...
async def test_conflicting_antecedent_periods_clarify() -> None:
    ...
    draft = KnowledgeRewrite(
        standalone="退货运费", referenced_turn_ids=[first.turn_id, second.turn_id]
    )
    service = FakeRetrieval()
    ctx = replace(context(responses=[draft]), retrieval=service)
    output = await invoke(
        ctx, inputs(question="那运费呢\uff1f", time_scope=None, knowledge_history=[first, second])
    )
...
@pytest.mark.parametrize(
    "draft_text", ["2025年8月退货运费规则", "2026年8月和9月退货运费规则", "2026年13月退货政策"]
)
...
async def test_required_rewrite_failure_is_typed_and_does_not_search_raw_followup() -> None:
    service = FakeRetrieval()
    ctx = replace(
        context(responses=[LlmStructuredOutputError("private-secret")]), retrieval=service
    )
    output = await invoke(
        ctx, inputs(question="那运费呢\uff1f", time_scope=None, knowledge_history=[topic()])
    )
...
async def test_query_used_recorded_in_evidence() -> None:
    prior = topic()
    ctx = replace(
        context(responses=[rewrite(prior, "退货相关运费承担规则")]),
        retrieval=FakeRetrieval(ranked()),
    )
    output = await invoke(
        ctx, inputs(question="那运费呢\uff1f", time_scope=None, knowledge_history=[prior])
...
async def test_old_evidence_keeps_original_unknown() -> None:
    ...
    query = RetrievalQuery(
        schema_version=1, standalone="legacy", time_scope=PointTimeScope(as_of=date(2026, 8, 1))
    )
...
async def test_checkpoint_preserves_new_query_and_clarification_contracts() -> None:
    ...
    state = KnowledgeAgentState(
        question="2026年13月规则", knowledge_history=[topic()], clarification=result.clarification
    )
    codec = serializer()
    assert codec.loads_typed(codec.dumps_typed(state)) == state
    with pytest.raises(ValidationError):
        KnowledgeAgentOutput(
            clarification=result.clarification, abstained=True, abstention_reason="no"
        )
...
async def test_new_time_span_and_history_are_masked() -> None:
    ...
    try:
        with service.turn(uuid4().hex, TraceMetadata()):
            await build().ainvoke(
                inputs(question="那运费呢\uff1f", time_scope=None, knowledge_history=[prior]),
