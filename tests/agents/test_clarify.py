...
from app.agents.contracts import PreparedContext, Route, RouteDecision, RouterInput
...
def clarification_state(
    ...
    *,
    question: str = "华东GMV（排除测试账号）",
    dimensions: list[MissingDimension] | None = None,
        ...
        **context().identity.model_dump(),
        ...
        route=RouteDecision(
            ...
            clarification_intent=ClarificationIntent(
                category=category,
                missing_dimensions=dimensions or [],
                metric_keys=["gmv"],
                subject=question,
            ),
        ),
...
async def test_out_of_scope_lists_capabilities() -> None:
    ...
    assert "GMV" in value.message
    assert "操作流程" in value.message
    assert "促销活动规则" in value.message
...
async def test_three_consecutive_clarifications_offers_interpretation() -> None:
    ...
    assert "替代解释" in value.message
    assert "2026年8月" in value.suggested_question
...
async def test_default_month_uses_request_shanghai_calendar(
...
async def test_missing_metric_preserves_explicit_period_and_region() -> None:
    ...
    result = await clarify(
        clarification_state(
            question="2024年7月华南的业务数据", dimensions=[MissingDimension.METRIC]
        ),
        Runtime(context=ctx),
    )
    value = result.update["route_clarification"]
...
async def test_knowledge_and_data_missing_dimensions_are_both_retained() -> None:
    ...
    assert result.update["route_clarification"].intent.missing_dimensions == [
        MissingDimension.METRIC,
        MissingDimension.PERIOD,
    ]
...
async def test_no_history_or_catalog_does_not_invent_capabilities() -> None:
    ...
    ctx = replace(
        ctx, clarification_capabilities=AsyncMock(read=AsyncMock(return_value=capabilities))
    )
    result = await clarify(
    ...
    assert "暂无已发布指标" in value.message
    assert "暂无已发布知识文档" in value.message
...
async def test_capability_failures_remain_failures(error: Exception) -> None:
    ctx = replace(
        context(responses=[]),
        clarification_capabilities=AsyncMock(read=AsyncMock(side_effect=error)),
    )
...
def test_v2_requires_policy_and_suggestion() -> None:
    with pytest.raises(ValidationError):
        MetricClarification(
            schema_version=2, kind=ClarificationKind.OUT_OF_SCOPE, message="invalid"
        )
...
async def test_document_only_capability_gives_document_example() -> None:
    ctx = replace(
        context(responses=[]),
        clarification_capabilities=AsyncMock(
            read=AsyncMock(
                return_value=ClarificationCapabilities(document_categories=[DocumentType.SOP])
            )
        ),
    )
    result = await clarify(
...
async def test_unsupported_metric_alternative_preserves_explicit_scope() -> None:
    ctx = context(responses=[])
    state = clarification_state(question="2024年7月华东的利润率")
    state.data_clarification = MetricClarification(
        kind=ClarificationKind.METRIC_NOT_FOUND, metric_key="profit_rate", message="unknown"
    )
    result = await clarify(state, Runtime(context=ctx))
    value = result.update["route_clarification"]
    assert "2024年7月华东" in value.suggested_question
    assert "将不支持的指标替换为GMV" in value.suggested_question
    assert "2026年8月" not in value.suggested_question


async def test_knowledge_period_suggestion_works_without_metrics() -> None:
    ctx = replace(
        context(responses=[]),
        clarification_capabilities=AsyncMock(
            read=AsyncMock(
                return_value=ClarificationCapabilities(document_categories=[DocumentType.POLICY])
            )
        ),
    )
    state = clarification_state(question="2026年13月华东退货政策")
    state.knowledge_clarification = KnowledgeClarification(
        kind=KnowledgeClarificationKind.PERIOD_UNRESOLVED, message="invalid date"
    )
    result = await clarify(state, Runtime(context=ctx))
    suggestion = result.update["route_clarification"].suggested_question
    assert "华东退货政策" in suggestion
    assert "统计期间改为2026年8月" in suggestion


async def test_third_turn_keeps_exact_default_period_bounds() -> None:
    ctx = context(responses=[])
    state = clarification_state(dimensions=[MissingDimension.PERIOD], consecutive=2)
    result = await clarify(state, Runtime(context=ctx))
    assert "2026-08-01 00:00 至 2026-09-01 00:00" in result.update["route_clarification"].message


async def test_repeated_reference_keeps_previous_supported_suggestion() -> None:
    ctx = context(responses=[])
    state = clarification_state(ClarificationCategory.AMBIGUOUS_REFERENCE, consecutive=2)
    state.prepared.clarification_history.previous_suggestion = "查询2026年8月退款率。"
    state.prepared.clarification_history.previous_metric_keys = ["refund_rate"]
    result = await clarify(state, Runtime(context=ctx))
    assert result.update["route_clarification"].suggested_question == "查询2026年8月退款率。"
    state.prepared.clarification_history.previous_metric_keys = ["removed_metric"]
    result = await clarify(state, Runtime(context=ctx))
    assert "GMV" in result.update["route_clarification"].suggested_question
