    ...
    try:
        with telemetry.turn(uuid4().hex, TraceMetadata()):
            result = await route_question(
                RouterInput(question=BOTH_QUESTION), runtime([decision(confidence=0.4)]),
            )
        ...
        assert BOTH_QUESTION not in json.dumps([dict(span.attributes) for span in spans], ensure_ascii=False)
...
async def test_unknown_route_exhausts_real_ladder_and_clarifies(respx_mock: respx.MockRouter) -> None:
    route = respx_mock.post(URL).mock(return_value=response('{"route":"invented","confidence":1}'))
    async with service() as llm:
        result = await route_question(RouterInput(question=BOTH_QUESTION), replace(runtime(), llm=llm))
    assert result.route is Route.CLARIFY
...
async def test_router_tokens_sum_all_tiers_and_repair(
    ...
    respx_mock.post(URL).mock(side_effect=[
        response("invalid"), response("invalid", tool_name="structured_response"),
        response("invalid"), response(decision().model_dump_json()),
    ])
    telemetry, exporter = tracing(settings)
    try:
        async with service() as llm:
        ...
        assert result.route is Route.BOTH
        assert router_span.attributes["langfuse.observation.metadata.router_tokens"] == str(FULL_LADDER * TOKENS_PER_RESPONSE)
...
async def test_unreported_usage_is_unknown(
    respx_mock: respx.MockRouter, monkeypatch: pytest.MonkeyPatch, missing: str,
) -> None:
    ...
    payload.pop("usage")
    replies = [httpx.Response(503), response(decision().model_dump_json())] if missing == "retry" else [httpx.Response(200, json=payload)]
    respx_mock.post(URL).mock(side_effect=replies)
    async with service() as llm:
        with collect_usage() as usage:
            await llm.generate_structured(
                role=ModelRole.ROUTER,
                messages=[], schema=type(decision()), deadline=runtime().deadline,
            )
    ...
    try:
        async with service() as llm:
            async def run(question: str) -> Route:
                with telemetry.turn(uuid4().hex, TraceMetadata()):
                    result = await route_question(RouterInput(question=question), replace(runtime(), llm=llm))
                    return result.route

            assert await asyncio.gather(run("为什么甲上涨"), run("为什么乙上涨")) == [Route.BOTH, Route.BOTH]
        telemetry.client.flush()
        totals = sorted(int(span.attributes["langfuse.observation.metadata.router_tokens"])
            for span in exporter.get_finished_spans() if span.name == "router")
        assert totals == [11, 31]
        with collect_usage() as later:
            assert later.attempts == 0 and later.total is None
    finally:
        await telemetry.aclose()
...
def test_routing_metadata_masks_prose_and_invalid_diagnostic_types() -> None:
    safe = mask({"route": "both", "original_route": "data_only", "confidence": 0.5,
        "router_tokens": 20, "prefilter_hit": False, "decided_by": "llm",
        "question": "private-question", "data_intent": "private-intent", "reasoning": "private-reason"})
    assert safe["router_tokens"] == TOKENS_PER_RESPONSE
    assert "private-" not in str(safe)
    for name in ("route", "original_route", "confidence", "decided_by", "prefilter_hit", "router_tokens"):
    ...
    assert mask({"router_tokens": True, "confidence": float("nan")}) == {
        "router_tokens": REDACTED, "confidence": REDACTED,
    }
