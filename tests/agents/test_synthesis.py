...
from app.agents.synthesis_answer import synthesis_answer, validate_result
...
from tests.agents.synthesis_support import synthesis_context, synthesis_draft, synthesized
...
@pytest.mark.parametrize("text", ["政策导致退款增加", "Refunds were caused by policy", "因为政策变化", "Changes resulted in refunds"])
@pytest.mark.parametrize("kind", [ClaimKind.FACT_DATA, ClaimKind.FACT_DOCUMENT])
async def test_causal_marker_on_fact_claim_downgraded_to_inference(text: str, kind: ClaimKind) -> None:
    ctx, state, bundle = await synthesis_context()
    claim = Claim(text=text, kind=kind, confidence=1,
        data_refs=[CellReference(row=0, column=0, value=42)],
        chunk_ids=[bundle.knowledge.knowledge.chunks[0].chunk_id])
    with capture_logs() as logs:
        output = await synthesized(replace(ctx, llm=FakeChatModel([SynthesisOutput(claims=[claim])])), state)
    assert output.claims[0].kind is ClaimKind.INFERENCE
...
async def test_statistic_reference_uses_all_returned_rows_not_sample() -> None:
    ...
    output = SynthesisOutput(claims=[Claim(text="合计 42。", kind=ClaimKind.FACT_DATA,
        confidence=1, data_refs=[StatisticReference(column=0, field="total", value="42")])])
...
async def test_successful_zero_rows_is_valid_data() -> None:
    ...
    draft = SynthesisOutput(claims=[Claim(text="查询返回 0 行。", kind=ClaimKind.FACT_DATA,
        data_refs=[RowCountReference(value=0)], confidence=1)])
    output = await synthesized(replace(ctx, llm=FakeChatModel([draft])), state)
    assert output.abstention is None
...
@pytest.mark.parametrize("conflict", [Conflict(left_claim=0, right_claim=0), Conflict(left_claim=0, right_claim=5)])
...
async def test_missing_fact_references_are_rejected() -> None:
    ...
    invalid = SynthesisOutput(claims=[Claim(text="政策明确。", kind=ClaimKind.FACT_DOCUMENT, confidence=1)])
...
async def test_expired_deadline_does_not_call_model() -> None:
    ctx, state, _ = await synthesis_context()
    command = await synthesize(state, Runtime(context=replace(ctx, deadline=Deadline(time.monotonic() - 1))))
...
async def test_malformed_snapshot_is_not_a_model_repair() -> None:
    ctx, state, bundle = await synthesis_context()
    ctx.evidence.snapshot = bundle.data.model_copy(update={"data": bundle.data.data.model_copy(update={"generation_block": "{}"})})
    command = await synthesize(state, Runtime(context=ctx))
    assert command.update["status"] == "failed"
    assert ctx.llm.calls == []
    with pytest.raises(SynthesisEvidenceError):
        validate_output(synthesis_draft(), synthesis_input("bad", await ctx.evidence.read_bundle(ctx.identity)))
...
async def test_table_preference_does_not_generate_new_prose() -> None:
    ...
    answer = synthesis_answer(output, bundle, [], FormatPreferenceContent(prefer="table", decimals=2))
...
async def test_t7_frozen_policy_exposes_missing_payment_and_usage_evidence() -> None:
    ...
    from tests.agents.synthesis_support import knowledge_text

    ctx, state, initial = await synthesis_context()
    rates = sanity_payload([["0.04"], ["0.06"]], columns=[ColumnSpec(name="refund_rate", type="numeric")])
    ctx.evidence.snapshot = initial.data.model_copy(update={"data": package_result(rates, ["相同观察截止的支付同期群退款率。"] )})
    policy = Path("data/corpus/promo_2026_summer.md").read_text()
    bundle = knowledge_text(ctx, policy)
    draft = synthesis_draft(knowledge_id=bundle.knowledge.knowledge.chunks[0].chunk_id)
    draft.claims[0].text = "退款率从 4% 上升至 6%。"
    draft.claims[0].data_refs = [CellReference(row=0, column=0, value="0.04"), CellReference(row=1, column=0, value="0.06")]
    draft.claims[1].text = "活动将先用后付扩展至服饰品类。"
    draft.claims.append(Claim(text="先用后付导致退款上升。", kind=ClaimKind.FACT_DOCUMENT,
        chunk_ids=draft.claims[1].chunk_ids, confidence=1))
    draft.unanswered = ["需要按支付方式分组的数据及实际先用后付使用记录。", "需要品类、退款原因和一致观察截止。"]
    output = await synthesized(replace(ctx, llm=FakeChatModel([draft])), state)
    answer = synthesis_answer(output, bundle, [])
...
async def test_changed_corpus_cannot_replace_snapshot_citations() -> None:
    ...
    ctx = replace(ctx, retrieval=AsyncMock(side_effect=AssertionError("live lookup")), mcp=AsyncMock())
...
async def test_empty_knowledge_snapshot_counts_as_missing_support() -> None:
    ...
    ctx, state, bundle = await synthesis_context()
    ctx.evidence.knowledge = bundle.knowledge.model_copy(update={"knowledge": package_evidence(ranked(0.1), ctx)})
...
async def test_reference_repair_cannot_renew_deadline(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    llm = AsyncMock(generate_structured=AsyncMock(return_value=synthesis_draft(knowledge_id=uuid4())))
