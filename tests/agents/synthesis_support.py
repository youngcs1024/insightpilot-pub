...
def synthesis_draft(
    *, data: bool = True, knowledge_id: UUID | None = None
) -> SynthesisOutput:
    claims = []
    if data:
        claims.append(Claim(
            text="GMV 为 42。", kind=ClaimKind.FACT_DATA, confidence=0.95,
            data_refs=[CellReference(row=0, column=0, value=42)],
        ))
    if knowledge_id:
        claims.append(Claim(
            text="政策说明退款规则。", kind=ClaimKind.FACT_DOCUMENT,
            chunk_ids=[knowledge_id], confidence=0.95,
        ))
...
async def synthesis_context(
    *, data: bool = True, knowledge: bool = True
    ...
    state = AgentState(
        **ctx.identity.model_dump(), question="为什么退款率上升?",
        evidence_refs=bundle.refs,
        route=RouteDecision(route=Route.BOTH, confidence=1, data_intent="计算退款", knowledge_intent="退款政策"),
    )
...
def knowledge_text(ctx: RuntimeContext, text: str) -> EvidenceBundle:
    ...
    chunk = saved.knowledge.chunks[0].model_copy(update={
        "original_text": text, "generation_text": text,
        "effective_from": date(2026, 8, 5), "effective_to": date(2026, 8, 21),
    })
    block = render_documents((chunk,))
    evidence = saved.knowledge.model_copy(update={
        "chunks": (chunk,), "generation_block": block,
        "generation_tokens": ctx.schema_token_counter.count(block),
        "time_scope": FrozenRangeScope(periods=(FrozenPeriod(start=date(2026, 7, 1), end=date(2026, 9, 1)),)),
    })
