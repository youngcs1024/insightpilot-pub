...
def test_instruction_like_chunk_flagged_not_silently_dropped(
    counter: SchemaTokenCounter, payload: str, flag: InjectionFlag
) -> None:
    result = retrieval()
...
async def test_second_fabrication_abstains_without_leaking_draft(
...
async def test_empty_evidence_never_calls_llm(
    counter: SchemaTokenCounter, budget_empty: bool
) -> None:
    evidence = package_evidence(
        retrieval() if budget_empty else retrieval([]), EvidenceConfig(max_tokens=1), counter
    )
    llm = FakeChatModel([])
    result = await KnowledgeGenerationService(llm).generate(evidence, deadline=deadline())
    expected = (
        KnowledgeAbstention.BUDGET_EXHAUSTED if budget_empty else KnowledgeAbstention.NO_EVIDENCE
    )
    assert result.abstention is expected
...
async def test_model_may_abstain_from_insufficient_evidence(counter: SchemaTokenCounter) -> None:
    ...
    result = await KnowledgeGenerationService(llm).generate(evidence, deadline=deadline())
    assert result.abstention is KnowledgeAbstention.UNSUPPORTED
    assert result.attempts == 1


@pytest.mark.parametrize(
    "failure", [asyncio.CancelledError(), LlmUnavailableError(), DeadlineExceededError()]
)
async def test_generation_failure_is_not_a_citation_retry(
    counter: SchemaTokenCounter, failure: BaseException
) -> None:
    evidence = package_evidence(retrieval(), EvidenceConfig(), counter)
...
async def test_deadline_is_shared_and_not_renewed_for_correction(
    counter: SchemaTokenCounter, monkeypatch: pytest.MonkeyPatch
) -> None:
    evidence = package_evidence(retrieval(), EvidenceConfig(), counter)
    remaining = [10.0]
...
def test_abstention_cannot_contain_rejected_passages() -> None:
    with pytest.raises(ValidationError):
        KnowledgeGeneration(
            passages=draft(uuid4()).passages,
            abstention=KnowledgeAbstention.FABRICATED_CITATION,
            attempts=2,
        )
...
def test_model_cannot_supply_trusted_source_metadata() -> None:
    with pytest.raises(ValidationError):
        KnowledgeDraft.model_validate(
            {
                "passages": [
                    {"text": "规则", "chunk_ids": [str(uuid4())], "document_title": "伪造来源"}
                ]
            }
        )
