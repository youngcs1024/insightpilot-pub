...
@pytest.mark.parametrize(("question", "day"), [
    ...
    ("今天政策", "2026-09-08"), ("今日政策", "2026-09-08"),
    ("目前政策", "2026-09-08"), ("当前政策", "2026-09-08"),
    ("现在政策", "2026-09-08"), ("昨天政策", "2026-09-07"),
    ("昨日政策", "2026-09-07"), ("前天政策", "2026-09-06"),
    ("明天政策", "2026-09-09"),
])
...
def test_half_open_ranges(question: str, start: str, end: str) -> None:
    ...
    assert [(item.start.isoformat(), item.end.isoformat()) for item in result.scope.periods] == [(start, end)]


@pytest.mark.parametrize("question", [
    "2026-02-29政策", "2026年13月政策", "2026年8月32日政策",
    "2026年7月和13月政策", "2026年7月和2026/08/01政策", "2026年8月上旬政策",
    "2026年8月至7月政策", "2026-08-31到2026-08-01政策", "最近0天政策",
    "过去-1天政策", "最近1.5天政策", "Q5政策", "第五季度政策", "2026年-1月政策",
    "近期政策", "2026年春节政策", "去年同期政策", "9999年12月政策", "0000年政策",
    "2026年8月和2025年9月以及10月政策", "9999-12-31政策",
])
...
@pytest.mark.parametrize("question", ["七天无理由退货政策", "30天售后政策", "SKU-A1023政策", "产品SKU-2026-08-01的规则", "收到货以后七天内申请退货"])
...
def test_explicit_reference_year_and_last_year_equivalent() -> None:
    ...
    reference = RangeTimeScope(periods=[PolicyPeriod(start=date(2024, 2, 1), end=date(2024, 3, 1), label="闰年二月")])
...
def test_scope_comparison_preserves_comparison_structure() -> None:
    ...
    assert same_scope(parse_time("2026-08-01", now=NOW).scope, parse_time("2026年8月1日", now=NOW).scope)


@pytest.mark.parametrize(("question", "expected"), [("那运费呢？", True), ("它有什么限制", True), ("当时的运费", True), ("What about shipping?", True), ("2026年8月退款政策是什么？", False), ("七天无理由退货政策", False)])
