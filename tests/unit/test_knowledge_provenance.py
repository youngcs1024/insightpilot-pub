...
async def test_bad_registered_provenance_is_typed_error(damage: str) -> None:
    ...
    chunk = SimpleNamespace(id=value.chunk_uuid, **value.model_dump(exclude={"schema_version"}), page=None)
    metadata = CorpusMetadata.model_validate({
        "title": "规则", "doc_type": "policy", "effective_from": "2026-01-01",
        "effective_to": None, "supersedes": None,
    })
    document = SimpleNamespace(business_metadata=metadata.model_dump_json(), source_path=registered.source_path)
    if damage == "metadata":
    ...
    session = SimpleNamespace(execute=AsyncMock(return_value=SimpleNamespace(all=lambda: [(chunk, document)])))
