...
def pipeline(monkeypatch: pytest.MonkeyPatch) -> RetrievalPipeline:
    ...
    manifest = SimpleNamespace(corpus_version="a" * 64, collection="kb_chunks", members=[1], encoding=EncodingProfile.from_metadata(model_metadata()))
    ...
    output = EmbedResult(request_id="test", ms=0, queue_ms=0, inference_ms=0, metadata=model_metadata(), dense=[encoded().dense], sparse=[encoded().sparse])
    store = fake_store()
    store.hybrid_search = AsyncMock(return_value=[candidate()])
    return RetrievalPipeline(SimpleNamespace(session=sessions), store, SimpleNamespace(embed=AsyncMock(return_value=output)), RetrievalSettings())


async def test_vector_query_is_encoded_once_and_metadata_preserved(pipeline: RetrievalPipeline) -> None:
    ...
    assert result.retrieval_config.pool == 20
...
async def test_encoding_failure_stops_before_search(pipeline: RetrievalPipeline, failure: Exception) -> None:
...
async def test_manifest_encoding_mismatch_fails_closed(pipeline: RetrievalPipeline, monkeypatch: pytest.MonkeyPatch) -> None:
    manifest = SimpleNamespace(corpus_version="a" * 64, collection="kb_chunks", members=[1], encoding=None)
    monkeypatch.setattr(DocumentRepository, "manifest", AsyncMock(return_value=manifest))
    with pytest.raises(RetrievalConfigurationError):
        await pipeline.retrieve(query(), deadline=deadline())


async def test_manifest_collection_mismatch_fails_closed(pipeline: RetrievalPipeline, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(DocumentRepository, "manifest", AsyncMock(return_value=SimpleNamespace(collection="other")))
    with pytest.raises(RetrievalConfigurationError):
        await pipeline.retrieve(query(), deadline=deadline())


async def test_full_identity_and_text_hash_checked_after_admission(pipeline: RetrievalPipeline, monkeypatch: pytest.MonkeyPatch) -> None:
    good = candidate()
...
async def test_outer_deadline_includes_admission(pipeline: RetrievalPipeline, monkeypatch: pytest.MonkeyPatch) -> None:
