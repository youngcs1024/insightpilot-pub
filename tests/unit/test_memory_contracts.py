...
def test_memory_content_types_roundtrip(kind: MemoryType, content: dict[str, object]) -> None:
    value = Memory.model_validate({**memory().model_dump(), "memory_type": kind, "content": content})
...
def test_finalized_context_rejects_missing_scope_or_exceeded_bounds(update: dict[str, object]) -> None:
    with pytest.raises(ValidationError):
