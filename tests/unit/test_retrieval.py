...
def test_rrf_k_is_explicit_in_config() -> None:
    assert RetrievalConfig().model_dump()["rrf_k"] == EXPECTED_RRF_K
...
def test_interval_normalization_preserves_original_labels() -> None:
    ...
    assert (
        expression
        == "((effective_from == -1 or effective_from < 20697) and (effective_to == -1 or effective_to > 20635))"
    )
...
async def test_single_arm_reuses_native_score_without_fusion() -> None:
    ...
    assert result[0].scores.dense == NATIVE_SCORE
...
async def test_all_arms_use_identical_time_scope_and_record_scores() -> None:
    ...
    assert result[0].scores.model_dump(exclude={"schema_version", "rerank"}) == {
        "dense": 0.8,
        "sparse_learned": 0.8,
        "sparse_bm25": 0.8,
        "rrf": 0.8,
...
async def test_production_does_not_issue_diagnostic_searches() -> None:
    ...
    assert result[0].scores.rrf == NATIVE_SCORE
...
@pytest.mark.parametrize(
    ("error", "expected"),
    [
        (TimeoutError(), RetrievalUnavailableError),
        (ParamError(message="bad parameter"), RetrievalConfigurationError),
    ],
)
async def test_sdk_failure_has_typed_error_and_no_retry(
    error: Exception, expected: type[Exception]
) -> None:
    operation = AsyncMock(side_effect=error)
    with pytest.raises(expected):
        await fake_store(hybrid_search=operation).hybrid_search(
...
async def test_diagnostic_failure_is_not_silently_omitted() -> None:
    with pytest.raises(RetrievalUnavailableError):
        await fake_store(
            hybrid_search=AsyncMock(return_value=[[raw_hit()]]),
            search=AsyncMock(side_effect=TimeoutError()),
        ).hybrid_search(
            encoded(),
            RetrievalConfig(record_arm_scores=True),
            query().time_scope,
            deadline=deadline(),
            timeout_s=10,
        )
...
async def test_cancellation_propagates() -> None:
    with pytest.raises(asyncio.CancelledError):
        await fake_store(
            hybrid_search=AsyncMock(side_effect=asyncio.CancelledError())
        ).hybrid_search(
            encoded(),
            RetrievalConfig(),
            query().time_scope,
...
async def test_expired_budget_does_not_hang() -> None:
    ...
    with pytest.raises(RetrievalUnavailableError):
        await fake_store(hybrid_search=operation).hybrid_search(
            encoded(), RetrievalConfig(), query().time_scope, deadline=deadline(-1), timeout_s=10
        )
