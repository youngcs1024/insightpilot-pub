...
def test_multiple_comparison_periods_keep_labels() -> None:
    args = dev_retrieve.parser().parse_args(["退款规则", "--period", "2026-07-01:2026-08-01", "--period", "2026-08-01:2026-09-01"])
    assert len(dev_retrieve.parse_query(args, NOW).time_scope.periods) == 2


@pytest.mark.parametrize("flags", [["--as-of", "2026-02-30"], ["--period", "bad"], ["--period", "2026-08-01:2026-07-01"]])
...
def test_time_modes_are_mutually_exclusive() -> None:
    with pytest.raises(SystemExit):
        dev_retrieve.parser().parse_args(["规则", "--as-of", "2026-08-01", "--period", "2026-07-01:2026-08-01"])
...
def test_diagnostic_defaults_and_explicit_override() -> None:
    ...
    assert config.record_arm_scores and config.use_bm25 and not config.use_dense
    assert not dev_retrieve.search_config(args, RetrievalConfig(record_arm_scores=False)).record_arm_scores
    args = dev_retrieve.parser().parse_args(["规则", "--no-record-arm-scores"])
    assert not dev_retrieve.search_config(args, RetrievalConfig(record_arm_scores=True)).record_arm_scores
...
def test_bm25_cli_success_uses_process_settings(monkeypatch: pytest.MonkeyPatch) -> None:
    settings = dev_retrieve.RetrievalProcessSettings.model_construct(retrieval=dev_retrieve.RetrievalSettings())
