...
def vector() -> VectorRow:
    return VectorRow(
        chunk=PreparedChunk(
            document_id=uuid4(), document_version="a" * 64, chunking_version="b" * 64,
            chunk_uuid=uuid4(), content_sha256="c" * 64, ordinal=0,
            heading_path="政策", char_len=2, content="规则", parent_content="完整规则",
        ),
        metadata=CorpusMetadata(title="政策", doc_type=DocumentType.POLICY,
            effective_from=date(2026, 1, 1), effective_to=None, supersedes=None),
        dense=[1.0, *([0.0] * 1023)], sparse_learned={1: 1.0},
    )


@pytest.mark.parametrize("receipt", [
    {}, {"ids": [], "insert_count": 1}, {"ids": [1], "insert_count": 0},
...
async def test_insert_serializes_business_bounds_and_server_bm25(vector: VectorRow) -> None:
    ...
    assert kwargs["timeout"] is None and kwargs["retry_times"] == 0
...
def physical(vector: VectorRow, pk: int) -> dict[str, object]:
    return {
        "pk": pk, "chunk_uuid": str(vector.chunk.chunk_uuid),
...
async def test_cleanup_pages_without_order_assumption_and_keeps_current_pk(vector: VectorRow) -> None:
    query = AsyncMock(side_effect=[[physical(vector, 300), physical(vector, 100)], [physical(vector, 200)], []])
    delete = AsyncMock(return_value={"delete_count": 2})
    repository = store(query=query, delete=delete)
    assert await repository.cleanup(vector.chunk.document_id, [200]) == 2
