...
def test_no_semantic_group_overlap(development: Dataset, frozen: Dataset) -> None:
    ...
    assert not {case.semantic_group_id for case in development.cases} & {
        case.semantic_group_id for case in frozen.cases
    }
    assert sum(not case.answerable for case in development.cases) == 2
    assert sum(not case.answerable for case in frozen.cases) == 2


def test_all_judgments_reference_actual_original_chunks(
    development: Dataset, frozen: Dataset
) -> None:
    ...
    for dataset in (development, frozen):
        ...
        assert all(
            item.content_sha256 == digest(chunks[item.chunk_id].content)
            for item in dataset.manifest.chunks
        )
...
def test_expired_policy_grade_zero(development: Dataset) -> None:
    ...
    assert all(
        item.grade == 0
        for item in july.judgments
        if metadata[item.chunk_id].source_path == "refund_policy_v3.md"
    )
    assert all(
        item.grade == 0
        for item in current.judgments
        if metadata[item.chunk_id].source_path == "refund_policy_v2.md"
    )
...
def test_same_policy_valid_for_historical_question(development: Dataset) -> None:
    ...
    july = next(item for item in development.labels if item.query_id == "r-001")
    assert any(
        item.grade >= 2 and paths[item.chunk_id] == "refund_policy_v2.md" for item in july.judgments
    )
    damaged = development.model_copy(deep=True)
    current = next(item for item in damaged.labels if item.query_id == "r-002")
    next(
        item for item in current.judgments if paths[item.chunk_id] == "refund_policy_v2.md"
    ).grade = 3
