...
def test_cli_collect_retains_eight_arms_but_control_is_pending(
    ...
    assert result == 2
    assert path.exists()
    assert path.with_suffix(".raw.json").exists()
...
def test_cli_selection_and_report_phases(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert (
        cli.main(
            ["ablation", "--phase", "report", "--measurements", str(raw), "--report", str(report)]
        )
        == 0
    )
...
async def test_manifest_mismatch_fails_before_eval(monkeypatch: pytest.MonkeyPatch) -> None:
    settings = SimpleNamespace(
        model_runtime=SimpleNamespace(precision="fp16"),
        database=object(),
    ...
    monkeypatch.setattr(
        retrieval_runtime,
        "validate_active",
        AsyncMock(side_effect=EvaluationError("manifest mismatch")),
    )
    close = AsyncMock()
    retrieve = AsyncMock()
