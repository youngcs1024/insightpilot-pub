...
def candidate_pool() -> list[Candidate]:
    return [
        Candidate(
            ...
            content_sha256=digest(f"Evidence {index}"),
            content=f"Evidence {index}",
            parent_content=f"Parent evidence {index}",
            heading_path="Rule",
            source_path=f"policy-{index}.md",
            doc_type="policy",
            milvus_pk=index,
            effective_from=None,
            effective_to=None,
...
def dataset(split: Split = Split.DEVELOPMENT) -> Dataset:
    ...
    cases = [
        RetrievalCase(
            ...
            time_scope=PointTimeScope(as_of=date(2026, 9, 1)),
            semantic_group_id="one",
            split=split,
            category="policy",
    ...
    labels = [
        Judgments(
            ...
            judgments=[
                Judgment(
                    chunk_id=item.chunk_uuid, grade=3 if index < 2 else 0, why="Scripted test only"
                )
    ...
    manifest = DatasetManifest(
        ...
        chunking_version="b" * 64,
        queries_sha256="d" * 64,
        judgments_sha256=dict.fromkeys(Split, "e" * 64),
...
def attempt(arm: Arm) -> Attempt:
    ...
    if config.use_rerank:
        response = RerankResult(
            request_id="scripted-eval",
            ms=10,
            queue_ms=2,
            inference_ms=8,
            metadata=metadata.model_copy(update={"precision": "fp32"})
            if arm is Arm.D_FP32
            else metadata,
        ...
        observed=ObservedRetrieval(
            trace=RetrievalTrace(
                ...
                encoded=EncodedQuery(
                    text=case.text, dense=[1.0, *([0.0] * 1023)], sparse={1: 1.0}, metadata=metadata
                ),
            ),
...
def measurements(*, control: bool = True, split: Split = Split.DEVELOPMENT) -> Measurements:
    result = Measurements(
        client_sha=SHA,
        source_dirty=False,
        dataset_identity=dataset(split).identity,
        corpus_version="c" * 64,
        split=split,
        server=Provenance(
            source_sha=SHA,
    ...
    if split is Split.FROZEN:
        development = measurements(control=False)
        selected = choose(evaluate(development, dataset(), require_control=False), dataset())
        result.selection_commit = "b" * 40
        result.selection_identity = digest(canonical(selected.model_dump(mode="json")))
    return result
