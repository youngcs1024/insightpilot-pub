...
from app.schemas.ingestion import ActiveManifest, RegisteredDocument, VersionMember, canonical, digest
...
def published_document(category: DocumentType = DocumentType.POLICY) -> RegisteredDocument:
    ...
    return RegisteredDocument(
        document_id=uuid4(), document_version="a" * 64, chunking_version=digest("{}"),
        source_path="rule.md", source_fingerprint="b" * 64, content_sha256="c" * 64,
        metadata=CorpusMetadata(
            title="测试规则", doc_type=category, effective_from=date(2026, 1, 1),
            effective_to=None, supersedes=None,
        ),
        chunk_count=1,
    )
...
def published_manifest(document: RegisteredDocument) -> ActiveManifest:
    ...
    return ActiveManifest(
        corpus_version=digest(canonical([
            (str(member.document_id), member.document_version, member.chunking_version)
        ])),
        members=[member], splitter_configs={document.chunking_version: "{}"}, collection="test",
    )
