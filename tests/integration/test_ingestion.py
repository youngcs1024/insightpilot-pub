...
async def harness(
    ...
    settings = ModelRuntimeClientSettings(auth_token=SecretStr("synthetic-ingestion-token"))
...
async def test_reingest_unchanged_is_noop(
...
async def test_changed_file_replaces_only_its_chunks(harness: Harness) -> None:
    ...
    assert result.chunks_deleted == len(old)
    assert await harness.chunks(document_id("other.md")) == other
    assert {item.chunk_uuid for item in await harness.chunks(document_id("rule.md"))}.isdisjoint(
        item.chunk_uuid for item in old
    )
...
async def test_deleted_file_chunks_removed(harness: Harness) -> None:
    ...
    assert not tombstone.cleanup_pending
...
async def test_repeated_text_preserves_provenance(harness: Harness) -> None:
    await harness.service().ingest(harness.root)
    first, second = (
        await harness.chunks(document_id("rule.md")),
        await harness.chunks(document_id("other.md")),
    )
...
async def test_registry_failure_preserves_previous_active_version(
    harness: Harness, monkeypatch: pytest.MonkeyPatch
) -> None:
...
async def test_uncommitted_manifest_not_retrievable(
    harness: Harness, monkeypatch: pytest.MonkeyPatch
) -> None:
    observed = False
...
async def test_registry_and_active_manifest_commit_atomically(
    harness: Harness, monkeypatch: pytest.MonkeyPatch
) -> None:
    original = DocumentRepository.save_manifest
...
async def test_crash_after_commit_cleanup_retry(
    harness: Harness, monkeypatch: pytest.MonkeyPatch
) -> None:
    await harness.service().ingest(harness.root)
    ...
    assert retry.documents_changed == 0
    assert retry.chunks_inserted == 0
    assert retry.chunks_deleted > 0
    assert len(harness.embeddings.calls) == calls


async def test_cleanup_failure_returns_pending(
    harness: Harness, monkeypatch: pytest.MonkeyPatch
) -> None:
...
async def test_candidate_physical_identity_must_match(harness: Harness) -> None:
    ...
    invalid = [
        ...
        for field, value in [
            ("milvus_pk", candidate.milvus_pk + 1),
            ("content_sha256", "0" * 64),
            ("document_version", "0" * 64),
            ("chunking_version", "0" * 64),
        ]
    ]
    async with harness.database.session() as reader:
...
async def test_complete_authored_corpus_is_idempotent(harness: Harness) -> None:
    ...
    assert first.documents_changed == 60
...
async def test_fresh_databases_produce_identical_chunk_ids(
    ...
    try:
        ...
        async with IngestionStore(
            ...
            assert [item.document_version for item in before] == [
                item.document_version for item in after
            ]
    finally:
        await database.aclose()
