...
from app.schemas.ingestion import document_id
...
from tests.retrieval_support import QueryEmbeddings, RetrievalHarness, corpus, deadline, encoded, query
...
@pytest.fixture
async def harness(migrated_db: TestPostgres, milvus_stack: MilvusStack, tmp_path: Path) -> AsyncIterator[RetrievalHarness]:
    ...
    try:
        ...
        async with (
            ...
            value = RetrievalHarness(database, ingestion, search, model, settings, embeddings, tmp_path)
            await value.ingest()
...
@pytest.mark.parametrize("dense,sparse,bm25", [
    (True, False, False), (False, True, False), (False, False, True),
    (True, True, False), (True, False, True), (False, True, True), (True, True, True),
])
async def test_all_enabled_arm_combinations_return_registered_candidates(harness: RetrievalHarness, dense: bool, sparse: bool, bm25: bool) -> None:
    config = RetrievalConfig(use_dense=dense, use_sparse_learned=sparse, use_bm25=bm25, record_arm_scores=True)
...
async def test_dense_only_returns_candidates(harness: RetrievalHarness) -> None:
    result = await harness.pipeline(RetrievalConfig(use_sparse_learned=False, use_bm25=False)).retrieve(query(), deadline=deadline())
    assert result.candidates and result.candidates[0].scores.dense is not None
...
async def test_parent_content_is_populated(harness: RetrievalHarness) -> None:
    result = await harness.pipeline(RetrievalConfig(use_dense=False, use_sparse_learned=False)).retrieve(query(), deadline=deadline())
    assert result.candidates
...
async def test_arm_scores_recorded_when_enabled(harness: RetrievalHarness) -> None:
    ...
    assert sku.scores.dense is not None and sku.scores.sparse_learned is not None
    assert sku.scores.sparse_bm25 is not None and sku.scores.rerank is None
...
async def test_comparison_retrieves_july_and_august_versions(harness: RetrievalHarness) -> None:
    request = RetrievalQuery(standalone="退款规则", time_scope=RangeTimeScope(periods=[
    ...
    assert {document_id("july.md"), document_id("august.md")} <= {item.document_id for item in result.candidates}
...
async def test_unbounded_dates_match(harness: RetrievalHarness) -> None:
    ...
    raw = await harness.search._client.query(harness.search.settings.collection, filter=f"pk == {pk}", output_fields=["*"], consistency_level="Strong", timeout=10)
    row = raw[0]
    row.pop("sparse_bm25", None)
    row["effective_from"], row["effective_to"] = -1, -1
    row.pop("pk")
    receipt = await harness.search._client.insert(harness.search.settings.collection, data=[row], timeout=10)
    scope = PointTimeScope(as_of=date(1900, 1, 1))
    result = await harness.search.hybrid_search(encoded(), RetrievalConfig(), scope, deadline=deadline(), timeout_s=30)
    assert any(item.milvus_pk in receipt["ids"] and item.effective_from is None and item.effective_to is None for item in result)
...
async def test_registry_rejects_stale_or_mismatched_hits(harness: RetrievalHarness, damage: str) -> None:
    identifier = document_id("sku.md")
    async with harness.database.session() as session, session.begin():
        if damage == "retired":
            await session.execute(update(Document).where(Document.id == identifier).values(status="deleted"))
        elif damage == "hash":
            await session.execute(update(Chunk).where(Chunk.document_id == identifier).values(content_sha256="f" * 64))
        else:
            await session.execute(update(Chunk).where(Chunk.document_id == identifier).values(milvus_pk=None))
    result = await harness.pipeline().retrieve(query(), deadline=deadline())
...
async def test_returned_manifest_matches_committed_registry(harness: RetrievalHarness) -> None:
    ...
    assert all(any(member.document_id == item.document_id and member.document_version == item.document_version and member.chunking_version == item.chunking_version for member in manifest.members) for item in result.candidates)


async def test_empty_candidates_are_not_an_error(harness: RetrievalHarness) -> None:
    request = RetrievalQuery(standalone="退款规则", time_scope=PointTimeScope(as_of=date(1900, 1, 1)))
    result = await harness.pipeline().retrieve(request, deadline=deadline())
    assert result.candidates == []
