...
async def test_dense_only_returns_candidates(harness: RetrievalHarness) -> None:
    result = await harness.pipeline(
        RetrievalConfig(use_rerank=False, use_sparse_learned=False, use_bm25=False)
...
async def test_parent_content_is_populated(harness: RetrievalHarness) -> None:
    result = await harness.pipeline(
        RetrievalConfig(use_rerank=False, use_dense=False, use_sparse_learned=False)
    ).retrieve(query(), deadline=deadline())
    assert result.candidates
...
async def test_bm25_arm_finds_exact_sku_code(harness: RetrievalHarness) -> None:
    ...
    dense = RetrievalConfig(use_rerank=False, use_sparse_learned=False, use_bm25=False, pool=1)
    lexical = RetrievalConfig(use_rerank=False, use_dense=False, use_sparse_learned=False, pool=1)
...
async def test_unbounded_dates_match(harness: RetrievalHarness) -> None:
    ...
    result = await harness.search.hybrid_search(
        encoded(), RetrievalConfig(use_rerank=False), scope, deadline=deadline(), timeout_s=30
    )
    assert any(
