...
async def history(database: Database, identity: TurnIdentity) -> list[StoredMemory]:
    async with database.session() as session:
        return await MemoryRepository(session, identity.user_id).list_history(
...
async def test_conflict_creates_and_supersedes(memory_db: Database, settings: Settings) -> None:
    ...
    assert old.model_dump(
        exclude={"is_active", "superseded_by", "superseded_at", "updated_at"}
    ) == original.model_dump(exclude={"is_active", "superseded_by", "superseded_at", "updated_at"})


async def test_superseded_row_preserved_with_pointer(
    ...
    assert any(
        row["event"] == "memory_same_turn_conflict" and row["log_level"] == "warning"
        for row in logs
    )
...
async def test_chain_of_three_supersessions_readable(
    memory_db: Database, settings: Settings
) -> None:
    identity = await source_pair(memory_db, extraction_input())
    outcomes = await service(memory_db, settings)._write(
        identity,
        MemoryExtraction(candidates=[candidate(), changed(), changed("o.created_at"), candidate()]),
    )
    rows = {row.id: row for row in await history(memory_db, identity)}
    assert len(rows) == 4
    for old, new in pairwise(outcomes):
...
async def test_terminology_fuzzy_duplicate_detected(
    memory_db: Database, settings: Settings
) -> None:
    identity = await source_pair(memory_db, extraction_input())
    ...
    results = await service(memory_db, settings)._write(
        identity, MemoryExtraction(candidates=[a, b])
    )
    assert results[1].status is WriteStatus.DUPLICATE
    async with memory_db.session() as session:
        rows = await MemoryRepository(session, identity.user_id).list_history(
...
async def test_distinct_keys_coexist(memory_db: Database, settings: Settings) -> None:
    ...
    results = await service(memory_db, settings)._write(
        identity, MemoryExtraction(candidates=values)
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("terminology", {"term": "大促", "means": "618"}, {"term": "大促", "means": "双11"}),
    ],
)
async def test_other_types_replace_and_preserve_original(
    ...
    assert not old.is_active
    assert len(active) == 1
    assert active[0].content == b.content


async def test_same_batch_conflict_after_duplicate_warns(
    memory_db: Database, settings: Settings
) -> None:
    first = await source_pair(memory_db, extraction_input())
    writer = service(memory_db, settings)
    await writer._write(first, MemoryExtraction(candidates=[candidate()]))
    second = await source_pair(memory_db, extraction_input(), owner=first)
    with capture_logs() as logs:
        outcomes = await writer._write(
            second, MemoryExtraction(candidates=[candidate(), changed()])
        )
    assert outcomes[0].status is WriteStatus.DUPLICATE
    assert outcomes[1].status is WriteStatus.SUPERSEDED
    assert any(event["event"] == "memory_same_turn_conflict" for event in logs)
