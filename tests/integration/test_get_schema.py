...
async def test_non_allowlisted_table_in_rejected_list(client: McpClient) -> None:
    response = await client.get_schema(
        GetSchemaArgs(tables=["biz.orders", "pg_catalog.pg_user"]), deadline=budget()
    )
...
async def test_include_samples_capped_at_three(client: McpClient) -> None:
    ...
    assert all(
        len(value) <= 50 for t in result.tables for c in t.columns for value in c.sample_values
    )
...
def test_artifact_matches_metadata_table(
    ...
    command = [
        ...
        "-m",
        "scripts.export_schema_artifact",
        "--output",
        str(output),
    ]
    exported = subprocess.run(  # noqa: S603 -- fixed CLI and fixture-owned output.
        command, env=env, capture_output=True, text=True, timeout=45, check=False
    )
    assert exported.returncode == 0, exported.stdout + exported.stderr
    assert output.read_bytes() == ARTIFACT.read_bytes()
    checked = subprocess.run(  # noqa: S603 -- fixed CLI.
        [*command, "--check"], env=env, capture_output=True, text=True, timeout=45, check=False
    )
    ...
    rejected = subprocess.run(  # noqa: S603 -- fixed CLI.
        [*command, "--check"], env=env, capture_output=True, text=True, timeout=45, check=False
    )
