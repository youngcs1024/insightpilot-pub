...
def snapshot(settings: RouteProcessSettings) -> Snapshot:
    ...
    paths = git_bytes(["ls-files", "app", "evals/harness", "evals/cli.py"]).decode().splitlines()
...
def committed_selection(config: Snapshot, path: Path = SELECTION) -> tuple[Selection, str]:
    ...
    if selected.fingerprint != config.fingerprint() or selected.arm is not config.production_strategy:
...
async def observe(inputs: RouterInput, ctx: RoutingRuntime, arm: RoutingStrategy) -> Observation:
    ...
    hit = arm is not RoutingStrategy.LLM_ONLY and prefilter(inputs.question, inputs.routing_context) is not None
    result = Observation(latency_ms=0, prefilter_hit=hit)
    ...
    result.tokens = 0 if usage.attempts == 0 and (hit or arm is RoutingStrategy.PREFILTER_ONLY) else usage.total
    return result


async def _attempts(
    raw: Measurements, cases: list[Case], settings: RouteProcessSettings, llm: LazyRoutingLlm
) -> None:
        ...
        for case in cases:
            for arm in rotated:
                ...
                observation = await observe(
                    RouterInput(question=case.question, routing_context=case.routing_context.model_copy(deep=True)),
                    ctx, arm,
                )
                raw.attempts.append(Attempt(case_id=case.id, arm=arm, repeat=repeat, observation=observation))
                logger.info(
                    "routing_evaluation_attempt", case_id=case.id, arm=arm.value,
                    repeat=repeat, failure_code=observation.failure_code,
                )
...
async def collect(options: Options) -> Measurements:
    ...
    raw = Measurements(
        run_id=uuid4().hex, created_at=datetime.now(UTC), split=options.split,
        repeats=options.repeats, case_ids=[c.id for c in cases], config=config, attempts=[],
        selection=selection, selection_commit=commit,
    )
    llm = LazyRoutingLlm(settings.llm)
    try:
        await _attempts(raw, cases, settings, llm)
    finally:
        await llm.aclose()
