...
def _predicted(attempt: Attempt) -> Route | None:
    ...
    return observation.decision.route if observation.decision and not observation.failure_code else None
...
def metrics(attempts: list[Attempt], cases: list[Case]) -> Metrics:
    ...
    return Metrics(
        ...
        misroute=Score(
            passed=sum(p is not None and p is not Route.CLARIFY and p is not c.expected for c, p, _ in pairs),
            total=len(pairs),
        ),
        both_misroute=Score(
            passed=sum(c.expected is Route.BOTH and p in {Route.DATA_ONLY, Route.KNOWLEDGE_ONLY} for c, p, _ in pairs),
            total=sum(c.expected is Route.BOTH for c, _, _ in pairs),
        ),
        clarification_precision=Score(
            passed=sum(p is Route.CLARIFY and c.clarification_kind == "ambiguous" for c, p, _ in pairs),
            total=sum(p is Route.CLARIFY and c.clarification_kind != "out_of_scope" for c, p, _ in pairs),
        ),
        out_of_scope_recall=Score(
            passed=sum(c.clarification_kind == "out_of_scope" and p is Route.CLARIFY for c, p, _ in pairs),
            total=sum(c.clarification_kind == "out_of_scope" for c, _, _ in pairs),
        ),
        prefilter_hit_rate=Score(passed=sum(o.prefilter_hit for _, _, o in pairs), total=len(pairs)),
        ...
        per_class={
            route: ClassScore(
                ...
                recall=Score(passed=matrix[i][i], total=sum(c.expected is route for c, _, _ in pairs)),
            )
            for i, route in enumerate(ROUTES)
        },
        confusion_matrix=matrix,
        no_decision_by_class={r: sum(c.expected is r and p is None for c, p, _ in pairs) for r in ROUTES},
...
def _values(metric: Metrics) -> dict[str, float | None]:
    values = {
        ...
        for name in (
            "accuracy", "misroute", "both_misroute", "clarification_precision",
            "out_of_scope_recall", "prefilter_hit_rate", "prefilter_accuracy",
        )
    }
...
def evaluate(raw: Measurements, cases: list[Case]) -> Report:
    ...
    grid = {(c.id, arm, n) for c in selected for arm in RoutingStrategy for n in range(1, raw.repeats + 1)}
    actual = [(a.case_id, a.arm, a.repeat) for a in raw.attempts]
    valid = (
        raw.complete and not raw.config.source_dirty
        and set(raw.case_ids) == expected_ids and len(raw.case_ids) == len(expected_ids)
        and set(actual) == grid and len(actual) == len(grid)
    )
    arms = {}
    for arm in RoutingStrategy:
        attempts = [a for a in raw.attempts if a.arm is arm and a.case_id in expected_ids]
        repeats = {n: metrics([a for a in attempts if a.repeat == n], selected) for n in range(1, raw.repeats + 1)}
        ...
        arms[arm] = ArmReport(overall=metrics(attempts, selected), per_repeat=repeats, variation=variation)
    return Report(
        measurements=raw, arms=arms, evidence_valid=valid,
...
def passes(metric: Metrics, threshold: float = 0.90) -> bool:
    ...
    return accuracy is not None and misroute is not None and accuracy >= threshold and misroute <= 0.05
...
def choose(report: Report) -> Literal[RoutingStrategy.HYBRID, RoutingStrategy.LLM_ONLY]:
    ...
    if (
        passes(llm)
        and llm.accuracy.rate is not None and hybrid.accuracy.rate is not None
        and llm.accuracy.rate > hybrid.accuracy.rate
        and llm.misroute.rate is not None and hybrid.misroute.rate is not None
        and llm.misroute.rate <= hybrid.misroute.rate
        and llm.both_misroute.rate is not None and hybrid.both_misroute.rate is not None
