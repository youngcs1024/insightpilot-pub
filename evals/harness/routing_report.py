...
from evals.harness.routing_contracts import ROUTES, Report, Split
...
def markdown(report: Report) -> str:
    ...
    lines = [
        ...
        "", "## Configuration", "", "```json", raw.config.model_dump_json(indent=2), "```", "",
        "Strategy selection: " + (raw.selection.arm.value if raw.selection else "development comparison"),
    ...
    for arm, result in report.arms.items():
        ...
        lines.extend([
            ...
            *[f"| {r.value} | {score(s.precision)} | {score(s.recall)} | {metric.no_decision_by_class[r]} |" for r, s in metric.per_class.items()],
            "", "### Confusion matrix (rows = truth, columns = prediction)", "",
            "| Truth | " + " | ".join(r.value for r in ROUTES) + " |",
            "| --- | --- | --- | --- | --- |",
            *["| " + r.value + " | " + " | ".join(str(v) for v in row) + " |" for r, row in zip(ROUTES, metric.confusion_matrix, strict=True)],
            "", "No-decision attempts stay in accuracy/recall denominators and are listed above.",
            "", "### Repeats", "", "| Repeat | Accuracy | Misroute | BOTH misroute |",
            "| --- | --- | --- | --- |",
            *[f"| {n} | {score(m.accuracy)} | {score(m.misroute)} | {score(m.both_misroute)} |" for n, m in result.per_repeat.items()],
            "", "| Metric | Mean | Sample standard deviation |", "| --- | --- | --- |",
            *[f"| {key} | {v.mean} | {v.sample_stddev} |" for key, v in result.variation.items()],
        ])
    lines.extend([
        "", "## Attempts", "", "| Case | Arm | Repeat | Route | Tokens | Latency ms | Failure |",
        "| --- | --- | --- | --- | --- | --- | --- |",
    ])
    for attempt in raw.attempts:
        o = attempt.observation
        route = o.decision.route.value if o.decision else "no_decision"
        lines.append(f"| {attempt.case_id} | {attempt.arm.value} | {attempt.repeat} | {route} | {o.tokens} | {o.latency_ms:.2f} | {o.failure_code or ''} |")
    if report.identical_arm_accuracy:
        lines.extend(["", "All arms have identical accuracy: investigate dataset discrimination before acceptance."])
    return "\n".join(lines) + "\n"
...
def write_report(report: Report, directory: Path) -> None:
    ...
    raw = report.measurements
    for suffix, payload in (("json", report.model_dump_json(indent=2) + "\n"), ("md", markdown(report))):
        with (directory / f"routing_{raw.run_id}.{suffix}").open("x", encoding="utf-8") as target:
            target.write(payload)
        (directory / f"routing_{raw.split.value}_latest.{suffix}").write_text(payload, encoding="utf-8")
...
def exit_code(report: Report, threshold: float = 0.90) -> int:
    ...
    provenance = raw.split is Split.DEVELOPMENT or (
        raw.selection is not None and bool(raw.selection_commit)
        and raw.selection.fingerprint == raw.config.fingerprint()
        and raw.selection.arm is raw.config.production_strategy
    )
    return int(
        not report.evidence_valid or raw.repeats < 3 or report.identical_arm_accuracy
        or not provenance or arm is RoutingStrategy.PREFILTER_ONLY
