...
from evals.harness.routing_contracts import SELECTION, Options, Report, Selection, Split
...
def select(path: Path) -> int:
    ...
    if (
        raw.split is not Split.DEVELOPMENT or not report.evidence_valid
        or report.identical_arm_accuracy or raw.repeats < 3 or raw.config != current
    ):
    ...
    if arm is not current.production_strategy:
        raise EvaluationError("Commit the winning production strategy and remeasure development first")
    selection = Selection(
        arm=arm, development_run_id=raw.run_id, development_sha=raw.config.git_sha,
        development_report_hash=digest(content), fingerprint=raw.config.fingerprint(),
    )
    SELECTION.write_text(yaml.safe_dump(selection.model_dump(mode="json"), sort_keys=False), encoding="utf-8")
