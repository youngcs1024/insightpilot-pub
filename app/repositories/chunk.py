...
class ChunkRepository:
    ...
    async def replace(self, identifier: UUID, values: list[RegisteredChunk]) -> None:
        ...
        for value in values:
            self.session.add(Chunk(
                id=value.chunk_uuid, document_id=value.document_id,
                document_version=value.document_version, chunking_version=value.chunking_version,
                ordinal=value.ordinal, heading_path=value.heading_path, page=value.page,
                content_sha256=value.content_sha256, char_len=value.char_len, milvus_pk=value.milvus_pk,
            ))
        await self.session.flush()

    async def list_document(self, identifier: UUID) -> list[RegisteredChunk]:
        """Return detached, position-ordered current provenance."""
        rows = (await self.session.scalars(select(Chunk).where(Chunk.document_id == identifier).order_by(Chunk.ordinal))).all()
        return [RegisteredChunk(
            chunk_uuid=row.id, document_id=row.document_id,
            document_version=row.document_version, chunking_version=row.chunking_version,
            ordinal=row.ordinal, heading_path=row.heading_path, page=row.page,
            content_sha256=row.content_sha256, char_len=row.char_len, milvus_pk=row.milvus_pk,
        ) for row in rows]
    ...
    async def admit(self, candidates: list[ChunkIdentity]) -> list[ChunkIdentity]:
        ...
        rows = (await self.session.execute(
            ...
            .where(Chunk.id.in_([item.chunk_uuid for item in candidates]), Document.status == "active",
        ...
        try:
            for row, payload in rows:
                ...
                if any(member.document_id == row.document_id and member.document_version == row.document_version
                       and member.chunking_version == row.chunking_version for member in manifest.members):
                    allowed.add((row.id, row.document_id, row.document_version, row.chunking_version,
                                 row.content_sha256, row.milvus_pk))
        except ValidationError as exc:
            raise IngestionRegistryError() from exc
        return [item for item in candidates if item.milvus_pk is not None and (
            item.chunk_uuid, item.document_id, item.document_version, item.chunking_version,
            item.content_sha256, item.milvus_pk,
        ) in allowed]
