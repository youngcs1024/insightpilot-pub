...
def is_clarification(turn: Turn) -> bool:
    ...
    return turn.clarification is not None and turn.status in {
        TurnStatus.SUCCEEDED, TurnStatus.ABSTAINED
    }
...
async def load_clarification_history(
    ...
    previous = (
        await repository.session.scalars(repository.previous_assistants(conversation_id, before_seq))
    ).all()
    consecutive = 0
    ...
    topics = list(dict.fromkeys(bounded_text(turn.content, 240) for turn in questions if turn.content.strip()))
    return ClarificationHistory(consecutive=consecutive, recent_topics=topics[:3])
