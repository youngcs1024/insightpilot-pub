...
class IngestionStore(MilvusRepository):
    ...
    async def insert(self, values: list[VectorRow]) -> InsertReceipt:
        ...
        for value in values:
            item = value.chunk
            data.append({
                "chunk_uuid": str(item.chunk_uuid), "document_id": str(item.document_id),
                "document_version": item.document_version, "chunking_version": item.chunking_version,
                "content_sha256": item.content_sha256, "heading_path": item.heading_path,
                "content": item.content, "parent_content": item.parent_content,
                "doc_type": value.metadata.doc_type.value,
                "effective_from": (value.metadata.effective_from - EPOCH).days,
                "effective_to": (value.metadata.effective_to - EPOCH).days if value.metadata.effective_to else -1,
                "dense": value.dense, "sparse_learned": value.sparse_learned,
            })
        raw = await self._rpc("insert", lambda: self._client.insert(
            self.settings.collection, data=data, timeout=None, retry_times=0, retry_on_rate_limit=False,
        ))
        try:
        ...
        if result.insert_count != len(values) or len(receipt.ids) != len(values) or len(set(receipt.ids)) != len(values):
    ...
    async def identities(self, identifier: UUID) -> list[ChunkIdentity]:
        ...
        seen: set[int] = {-1}
        while True:
            raw = await self._rpc("query_document", lambda: self._client.query(
                self.settings.collection, filter="document_id == {document} and pk not in {seen}",
                filter_params={"document": str(identifier), "seen": sorted(seen)},
                output_fields=["pk", "chunk_uuid", "document_id", "document_version", "chunking_version", "content_sha256"],
                limit=1000, consistency_level="Strong",
                timeout=None, retry_times=0, retry_on_rate_limit=False,
            ))
            try:
                rows = TypeAdapter(list[PhysicalRow]).validate_python(raw)
                batch = [ChunkIdentity(
                    chunk_uuid=row.chunk_uuid, document_id=row.document_id,
                    document_version=row.document_version, chunking_version=row.chunking_version,
                    content_sha256=row.content_sha256, milvus_pk=row.pk,
                ) for row in rows]
            except ValidationError as exc:
            ...
            if keys & seen or len(keys) != len(rows) or any(row.document_id != identifier for row in rows):
    ...
    async def cleanup(self, identifier: UUID, keep: list[int]) -> int:
        ...
        for offset in range(0, len(stale), 1000):
            await self._rpc("delete_stale", lambda offset=offset: self._client.delete(
                self.settings.collection, ids=stale[offset:offset + 1000], timeout=None,
                retry_times=0, retry_on_rate_limit=False,
            ))
        return len(stale)
