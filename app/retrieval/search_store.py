...
from app.retrieval.fusion import (
    OUTPUT_FIELDS, SearchArm, arm_request, attach_scores, candidates, enabled_arms, time_filter,
)
...
class HybridSearchStore(MilvusRepository):
    ...
    async def hybrid_search(
        self, query: EncodedQuery, config: RetrievalConfig, scope: KnowledgeTimeScope,
        *, deadline: Deadline, timeout_s: float,
    ) -> list[Candidate]:
    ...
    async def _single(
        self, arm: SearchArm, query: EncodedQuery, config: RetrievalConfig, expression: str,
    ) -> list[Candidate]:
        ...
        with observe("retrieval_arm", TraceMetadata(tool=arm.value)):
            raw = await self._rpc(
                ...
                lambda: self._client.search(
                    self.settings.collection, data=request.data, anns_field=request.anns_field,
                    search_params=request.param, filter=expression, limit=config.pool,
    ...
    async def _search(
        self, query: EncodedQuery, config: RetrievalConfig, expression: str,
    ) -> list[Candidate]:
        ...
        with observe("retrieval_fusion", TraceMetadata(tool="rrf")):
            raw = await self._rpc(
                ...
                lambda: self._client.hybrid_search(
                    self.settings.collection, reqs=requests, ranker=RRFRanker(k=config.rrf_k),
                    limit=config.pool, output_fields=OUTPUT_FIELDS, consistency_level="Strong",
                    timeout=None, retry_times=0, retry_on_rate_limit=False,
                ),
            )
