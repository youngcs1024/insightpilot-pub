...
_DIGITS = {char: value for value, chars in enumerate(("零〇", "一", "二两", "三", "四", "五", "六", "七", "八", "九")) for char in chars}
_QUARTERS_IN_YEAR = 4
...
def year_of(scope: KnowledgeTimeScope) -> int | None:
    ...
    years = {bound.year for item in scope.periods for bound in (item.start, item.end - timedelta(days=1))}
...
def _month(year: int, month: int, label: str) -> PolicyPeriod:
    return period_scope(resolve_period(f"{year}年{month}月", now=datetime(year, 1, 1, tzinfo=BUSINESS_TZ))).model_copy(update={"label": label})
...
def calendar_atom(expression: str, *, now: datetime, year: int) -> PolicyPeriod:
    ...
    if re.fullmatch(rf"{YEAR}年", expression):
        ...
        return PolicyPeriod(start=date(actual_year, 1, 1), end=date(actual_year + 1, 1, 1), label=expression)
    raise PeriodUnresolved()


def relative_atom(expression: str, *, now: datetime, reference: KnowledgeTimeScope | None) -> list[PolicyPeriod]:
    """Relative words always use the request clock, except explicit last-year references."""
    offsets = {"今天": 0, "今日": 0, "当前": 0, "目前": 0, "现在": 0, "昨天": -1, "昨日": -1, "前天": -2, "明天": 1}
    ...
    if expression == "去年同期":
        ...
        return [period_scope(resolve_period(expression, now=now, reference_period=Period(
