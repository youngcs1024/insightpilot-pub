...
from app.core.errors import (
    IngestionAlreadyRunning, IngestionConfigurationError, InsightPilotError,
)
...
from app.schemas.ingestion import (
    ActiveManifest, DocumentStatus, EncodingProfile, IngestionResult, RegisteredChunk,
    RegisteredDocument, VectorRow, VersionMember, canonical, digest,
)
...
class IngestionService:
    ...
    def __init__(
        self, database: Database, store: IngestionStore, model: ModelRuntimeClient,
        settings: IngestionSettings, model_settings: ModelRuntimeClientSettings,
    ) -> None:
    ...
    async def ingest(self, root: Path) -> IngestionResult:
        ...
        async with asyncio.timeout(self.settings.timeout_s), self.database.session() as guard, guard.begin():
            if not await DocumentRepository(guard).try_lock():
                raise IngestionAlreadyRunning()
            logger.info("ingestion_started")
            result = await self._ingest(root, deadline)
            logger.info("ingestion_finished", changed=result.documents_changed,
                inserted=result.chunks_inserted, deleted=result.chunks_deleted,
                failed=len(result.failed_files), pending=len(result.cleanup_pending))
            return result
    ...
    async def _ingest(self, root: Path, deadline: Deadline) -> IngestionResult:
        ...
        result = IngestionResult(failed_files=plan.failed, corpus_version=manifest.corpus_version if manifest else None)
    ...
    async def _encode(
        self, plan: IngestionPlan, manifest: ActiveManifest | None, deadline: Deadline,
    ) -> tuple[list[VectorRow], ModelMetadata | None]:
        candidates = [(chunk, item.document.metadata) for item in plan.changed for chunk in item.chunks]
        rows = []
        ...
        for start in range(0, len(candidates), batch_size):
            ...
            response = await self.model.embed([chunk.content for chunk, _ in batch], EmbedMode.DOCUMENT, deadline=deadline)
            ...
            rows.extend(VectorRow(chunk=chunk, metadata=business, dense=dense, sparse_learned=sparse)
                for (chunk, business), dense, sparse in zip(batch, response.dense, response.sparse, strict=True))
    ...
    async def _publish(
        self, plan: IngestionPlan, previous: list[RegisteredDocument],
        old_manifest: ActiveManifest | None, metadata: ModelMetadata | None,
    ) -> ActiveManifest:
        ...
        ) for item in sorted(documents.values(), key=lambda item: str(item.document_id)) if item.status is DocumentStatus.ACTIVE]
        ...
        manifest = ActiveManifest(
            corpus_version=digest(canonical([(str(item.document_id), item.document_version, item.chunking_version) for item in members])),
            members=members, splitter_configs={key: value for key, value in configurations.items() if key in {item.chunking_version for item in members}},
            encoding=EncodingProfile.from_metadata(metadata) if metadata else None,
            model_metadata=metadata, collection=self.store.settings.collection,
        )
        ...
        async with self.database.session() as session, session.begin():
            ...
            for value in plan.changed:
                await chunks.replace(value.document.document_id, [RegisteredChunk.model_validate(
                    chunk.model_dump(include=set(RegisteredChunk.model_fields)),
                ) for chunk in value.chunks])
            await repository.save_manifest(manifest)
        logger.info("ingestion_manifest_committed", corpus_version=manifest.corpus_version, members=len(members))
        return manifest

    async def _cleanup(self, result: IngestionResult) -> None:
        async with self.database.session() as session:
            pending = [item for item in await DocumentRepository(session).list_documents() if item.cleanup_pending]
