...
from app.schemas.ingestion import (
    DocumentStatus, FileFailure, PreparedDocument, RegisteredDocument, document_id,
)
...
def stage(root: Path, previous: list[RegisteredDocument], config: IngestionSettings) -> IngestionPlan:
    ...
    plan = IngestionPlan(deleted=[
        item.model_copy(update={"status": DocumentStatus.DELETED, "chunk_count": 0, "cleanup_pending": True})
        for item in previous if item.document_id not in present and item.status is DocumentStatus.ACTIVE
    ])
    for entry in inventory.documents:
        try:
            ...
            if old is not None and old.status is DocumentStatus.ACTIVE and old.source_fingerprint == fingerprint and old.chunking_version == config.chunking_version():
                continue
            prepared = prepare(load_source(entry, source.raw, source.sidecar, fingerprint), config)
            if old is not None and old.status is DocumentStatus.ACTIVE and old.document_version == prepared.document.document_version and old.chunking_version == prepared.document.chunking_version:
