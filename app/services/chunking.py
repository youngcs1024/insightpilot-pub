...
from app.schemas.ingestion import (
    LoadedSource, PreparedChunk, PreparedDocument, RegisteredDocument, SourcePart,
    canonical, digest, document_id, normalize,
)
...
def recursive(size: int, overlap: int) -> RecursiveCharacterTextSplitter:
    ...
    return RecursiveCharacterTextSplitter(
        ...
        keep_separator=True, strip_whitespace=True, is_separator_regex=False,
    )
    ...
    for part in source.parts:
        if source.entry.format is CorpusFormat.PDF:
            result.extend(
                ...
                for text in recursive(config.parent_size, config.parent_overlap).split_text(part.text)
            )
        else:
            ...
            for section in splitter.split_text(part.text):
                headers = [
                    ...
                    for level in range(1, 4) if f"h{level}" in section.metadata
                ]
                result.append(SourcePart(
                    text="\n".join(headers) + "\n\n" + section.page_content if headers else section.page_content,
                    heading=" / ".join(str(section.metadata[key]) for key in ("h1", "h2", "h3") if key in section.metadata),
                ))
    return result
...
def prepare(source: LoadedSource, config: IngestionSettings) -> PreparedDocument:
    ...
    text = normalize("\n\n".join(
        ("## " + part.heading + "\n\n" if part.heading else "") + part.text for part in source.parts
    ))
    identifier = document_id(source.entry.path)
    version = digest(canonical({"content": text, "metadata": source.metadata.model_dump(mode="json")}))
    ...
    try:
        for parent in contexts(source, config):
            ...
            for content in children:
                identity = canonical({"document_version": version, "chunking_version": chunking,
                    "heading_path": parent.heading, "ordinal": len(chunks), "content_sha256": digest(content)})
                parent_text = ("## " + parent.heading + "\n\n" + parent.text) if source.entry.format is CorpusFormat.EXCEL else parent.text
                if any(len(value.encode("utf-8")) > limit for value, limit in [(content, 65535), (parent_text, 65535), (parent.heading, 512)]):
                    raise CorpusValidationError("Chunk exceeds storage byte limit.")
                chunks.append(PreparedChunk(
                    document_id=identifier, document_version=version, chunking_version=chunking,
                    chunk_uuid=uuid5(identifier, identity), content_sha256=digest(content),
                    ordinal=len(chunks), heading_path=parent.heading, page=parent.page,
                    char_len=len(content), content=content, parent_content=parent_text,
                ))
        return PreparedDocument(document=RegisteredDocument(
            document_id=identifier, source_path=normalize(source.entry.path),
            source_fingerprint=source.source_fingerprint, content_sha256=digest(text),
            document_version=version, chunking_version=chunking, metadata=source.metadata,
