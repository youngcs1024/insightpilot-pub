...
def _final_time(
    state: KnowledgeAgentState, ctx: RuntimeContext, selected: list[KnowledgeHistoryTurn]
) -> KnowledgeTimeResolution:
    ...
    elif resolved.scope is None:
        resolved.scope = reference
        resolved.assumptions.append(
            "当前追问未指定新的时间，沿用所指上文的政策适用时间（Asia/Shanghai）。"
        )
    return resolved
...
def _query(
    ...
    if resolved.clarification is not None:
        return Command(
            update={"query_clarification": resolved.clarification, "rewritten": rewritten},
    ...
    checked = parse_time(
        standalone, now=ctx.now, reference=shared_scope(selected), reference_year=year_of(scope)
    )
    if checked.clarification is not None or (
        checked.scope is not None and not same_scope(checked.scope, scope)
    ):
        return Command(
            update={"query_clarification": time_clarification(), "rewritten": rewritten},
            goto="finish_knowledge",
        )
    return Command(
        update={
...
async def rewrite_query(
    ...
    if resolution.clarification is not None:
        return Command(
            update={"query_clarification": resolution.clarification}, goto="finish_knowledge"
        )
    ...
    if not history:
        return Command(
            update={"query_clarification": reference_clarification()}, goto="finish_knowledge"
        )
    rewritten = await ctx.llm.generate_structured(
        ModelRole.ROUTER,
        [
            SystemMessage(content=KNOWLEDGE_REWRITE),
            HumanMessage(
                content=json.dumps(
                    {
    ...
    update_current_observation(
        TraceMetadata(
            referenced_prior_turn=bool(selected),
    ...
    if not selected:
        return Command(
            update={"rewritten": rewritten, "query_clarification": reference_clarification()},
            goto="finish_knowledge",
        )
