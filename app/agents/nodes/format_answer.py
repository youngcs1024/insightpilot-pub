...
def _no_evidence(state: AgentState) -> Command[str]:
    ...
    if clarification is not None:
        return Command(
            update={"clarification": clarification, "abstained": True, "status": "abstained"},
            goto=END,
        )
    answer = Answer(
    ...
    return Command(update={"answer": answer, "abstained": True, "status": "abstained"}, goto=END)
...
async def _data_draft(
    ...
    if data.row_count:
        ...
        question = (
            state.route.data_intent
            if state.route
            else state.rewritten.standalone
            if state.rewritten
            else state.question
        )
...
async def _knowledge_draft(
    state: AgentState, ctx: RuntimeContext, bundle: EvidenceBundle
) -> KnowledgeGeneration | None:
...
def _assemble(
    ...
    if both:
        pieces.append(
            ...
            + (
                ...
                if missing
                else "以上分别呈现数据与文档事实，尚未建立因果关系；需要进一步的归因证据。"
            )
        )
    ...
    return Answer(
        markdown=markdown,
        confidence=min(confidence) if confidence else 0,
...
async def format_answer(state: AgentState, runtime: Runtime[RuntimeContext]) -> Command[str]:
    ...
    try:
        ...
        return Command(
            update={"answer": answer, "status": status, "abstained": answer.abstained}, goto=END
        )
