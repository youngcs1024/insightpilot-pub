...
_POLICY = re.compile(r"政策|规则|规定|流程|制度|\b(?:policy|policies|rules?|procedures?)\b", re.IGNORECASE)
_DEFINITION = re.compile(r"定义|口径|怎么算|怎么计算|如何计算|计算方法|是什么|含义|意思|解释|\b(?:definition|define|meaning|explain|calculated)\b", re.IGNORECASE)
_REFERENCE = re.compile(r"那个|这个|上次|刚才|之前|上述|那.{0,8}呢|\b(?:that|it|previous|above|those)\b", re.IGNORECASE)
_BARE_REFERENCE = re.compile(r"(?:请|帮我|再|看看|看一下|处理一下|分析一下|昨天|上次|刚才|之前|的|问题|那个|这个|吧|呢|\s|[\uff1f?\uff01!。.]|that|it|one|please|again)+", re.IGNORECASE)
_PERIOD = re.compile(
...
def prefilter(question: str, context: RoutingContext | None = None) -> RouteDecision | None:
    ...
    has_history = context is not None and (
        bool(context.summary.strip()) or any(item.content.strip() for item in context.recent_messages)
    )
    if not question or (_REFERENCE.search(question) and _BARE_REFERENCE.fullmatch(question)):
        if question and has_history:
            return None
        return RouteDecision(
            route=Route.CLARIFY, confidence=0.9, decided_by="prefilter",
    ...
    if metrics and period and not policy:
        return RouteDecision(
            route=Route.DATA_ONLY, confidence=0.95, decided_by="prefilter",
            data_intent=question, metric_hints=metrics,
        )
    if policy and not metrics and not period:
        return RouteDecision(
            route=Route.KNOWLEDGE_ONLY, confidence=0.95, decided_by="prefilter",
