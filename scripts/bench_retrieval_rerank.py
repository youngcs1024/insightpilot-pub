...
from scripts.model_evidence import CANDIDATE_COUNT, PAIR_COUNT, RERANK_P50_SECONDS, Evidence, Provenance, Seconds
...
class RerankBenchmark(Evidence):
    ...
    def p50_s(self) -> float | None:
        ...
        return statistics.median(item.client_seconds for item in self.measurements) if self.measurements else None
    ...
    def accepted(self) -> bool:
        ...
        return (
            ...
            and self.metadata.rerank_max_length == 320
            and self.metadata.rerank_batch == 16
            and len(self.measurements) == PAIR_COUNT
            and self.p50_s is not None and self.p50_s <= RERANK_P50_SECONDS
            and all(
                item.result.reranked and item.result.degradation is None
...
def workload() -> list[Candidate]:
    ...
    return [Candidate(
        ...
        content_sha256=digest(pair.passage), milvus_pk=index,
        content=pair.passage, parent_content="父章节。" + pair.passage,
        heading_path="固定业务记录", doc_type=DocumentType.POLICY,
        effective_from=None, effective_to=None, source_path=f"benchmark/{index}.md",
...
async def benchmark(client_sha: str, server: Provenance) -> RerankBenchmark:
    ...
    try:
        ...
        for _ in range(PAIR_COUNT):
            ...
            safe.candidates = []
            measurements.append(StageMeasurement(
                client_seconds=elapsed, rerank_seconds=result.stages[0].elapsed_ms / 1000,
                result=safe,
            ))
            if not result.reranked:
                break
        return RerankBenchmark(client_sha=client_sha, server=server, workload_sha256=input_hash(),
                               config=config, metadata=metadata, measurements=measurements)
    finally:
        await client.aclose()
...
def main() -> None:
    ...
    print(f"Rerank stage: accepted={result.accepted}; calls={len(result.measurements)}; "
